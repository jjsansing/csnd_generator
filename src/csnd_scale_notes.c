...
#define CSG_HEADER "# These staff definitions were generated by the Scale Notes Generator\n\n"
...
int *modify_staffnote(char *line, snScale *scale)
{
    ...
    if ((note = (snStaffNote *) sn_alloc(sizeof(snStaffNote), "staff note data")) == NULL)
        goto pcn_error;
...
int create_stafffile()
{
    ...
    while (scale != NULL)
    {
        ...
        while (staff != NULL)
        {
            ...
            while (chnote != NULL)
            {
                strcpy(cbuf, chnote->line);
                if (modify_staffnote(cbuf, scale) != 0)
                {
