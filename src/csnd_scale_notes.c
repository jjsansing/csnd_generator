...
char sn_Wintervals[] = {" ABCDEFGHIJKLMNOPQRSTUVWXY"};
...
int sn_test_os(void)
{
    ...
    {
        printf("Error: failed to open filename test file test.a\n  %s\n", strerror(errno));
        goto to_error;
    }
    if (fputs(test1, tfd) != 0)
    {
        CFILE_ERROR(tfd, "test 1 write", "test.a", to_error)
    }
    fclose(tfd);
    ...
    if (fputs(test2, tfd) != 0)
    {
        CFILE_ERROR(tfd, "test 1 write", "test.A", to_error)
    }
    fclose(tfd);

    if ((tfd = fopen("test.a", "r")) == NULL)
    {
        printf("Error: failed to open filename test file test.a\n  %s\n", strerror(errno));
        goto to_error;
    }
    ...
    {
        if (fgets(line, 16, tfd) == NULL)
        {
            if (feof(tfd))
                break;
            else if (ferror(tfd))
            {
                printf("Error: attempting to read from test.a\n  %s\n", strerror(errno));
                goto to_error;
            }
        }
    }
    if (strcmp(line, test1) != 0)
    {
        sn_os_win = 1;
        strcpy(sn_intervals, sn_Wintervals);
    }
    fclose(tfd);
...
int create_staff_file()
{
    ...
    while (scale != NULL)
    {
        ...
        if (gsz >= sn_group_size)
        {
            ...
            {
                CFILE_ERROR(vfd, "staff definition", fbuf, ccf_error)
            }
            ...
            while (header != NULL)
            {
                ...
                {
                    CFILE_ERROR(vfd, "header comment", fbuf, ccf_error)
                }
                header = header->next;
            }
        }
        ...
        while (staff != NULL)
        {
            ...
            {
                CFILE_ERROR(vfd, "instrument definition", fbuf, ccf_error)
            }
            stnote = staff->notes;
            while (stnote != NULL)
            {
                ...
                {
                    CFILE_ERROR(vfd, "note definition", fbuf, ccf_error)
                }
                else if (stnote->flag == MOD_COMMENT && modify_comment(cbuf, scale) != 0)
                {
                    CFILE_ERROR(vfd, "comment modification", fbuf, ccf_error)
                }
                if (fputs(cbuf, vfd) != 0)
                {
                    CFILE_ERROR(vfd, "note definition", fbuf, ccf_error)
        ...
        while ((opt = getopt(argc, argv, "d:l:n:v:f:g:i:km:s:x:")) != -1)
        {
           ...
           */
              default:
                  ...
                  {
                    printf("    A = half tone, B = whole tone, C = minor 3rd, D = major 3rd, E = fourth,\n");
                    printf("    F = diminished fifth, G = fifth, H = augmented fifth, I = sixth,\n");
                    printf("    J = augmented sixth, K = seventh, L = major seventh, M = eighth,\n");
                    printf("    N = diminished ninth, O = ninth, P = diminished tenth, Q = tenth,\n");
                    printf("    R = diminished eleventh, S = eleventh, T = diminished twelfth, U = twelfth,\n");
                    printf("    V = diminished thirteenth, W = thirteenth, X = diminished fourteenth, Y = fourteenth\n");
                    printf("    Example: csg_staves.BADACBB\n");
                    printf("  NOTE: This OS does not support mixed case file names\n");
                  }
                  printf("\n");
                  return 0;
                  break;
