  ...
  - int: Status
    ...
    # Check for blank lines and comments
        ...
        if (td[0] == "FUNC"):
            ...
            iname = "i" + td[1]
            instr = csnd_genobjP3.csgInstrument(iname)
            instrs.append(instr)
  ...
  - int: Status
    ...
    # Check for blank lines and comments
        ...
        if (cd[0] == "TEMPO"):
            ...
            cname = cd[1]
            staff = csnd_genobjP3.csgStaff(cname)
            staffs.append(staff)
    ...
    while (idx < len(staffs)):
        ...
        for nt in ch.notes:
            ...
            for gt in ch.instr.tones:
                ...
                fltstr = '%(f1)1.4f  %(f2)1.4f  %(f3)1.4f %(f4)1.4f' % \
                        {'f1': start, 'f2': durat, 'f3': vol, 'f4': float(freq)}
                wstr = ch.instr.name + "  " + fltstr + "  " + gt[r] + "\n"
            ...
            for st in ch.instr.stones:
                ...
                fltstr = '%(f1)1.4f  %(f2)1.4f  %(f3)1.4f %(f4)1.4f' % \
                        {'f1': start, 'f2': durat, 'f3': vol, 'f4': float(freq)}
                wstr = ch.instr.name + "  " + fltstr + "  " + str(st[F+1]) + "\n"
            ...
            for vt in ch.instr.vtones:
                ...
                if (vowel == -1):
                    return -1
                fltstr = '%(f1)1.4f  %(f2)1.4f  %(f3)1.4f %(f4)1.4f' % \
                        {'f1': start, 'f2': durat, 'f3': vol, 'f4': freq}
                wstr = ch.instr.name + "  " + fltstr + "  " + str(vowel) + "  " + str(vt[f+1]) + "\n"
...
Outputs:
  - int: Status
    ...
    print('\n')
    print('  csnd_genP3.py Version', csg_ver)
    print('\n')
    print('  csnd_genP3.py [arguments]:')
...
if __name__ == '__main__':
    ...
    if parse_notes() != 0:
        ...
        notes = csnd_genobjP3.csgNote(note_defs)
    if (debug & 1 == 1):
    ...
    if parse_template() != 0:
        ...
        vowels = csnd_genobjP3.csgVowel(vowel_defs)
    if (debug & 2 == 2):
